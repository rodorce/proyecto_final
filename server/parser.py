...
def p_PARAM(p):
    ...
    if name[0] not in varsTables[activeFuncTable]:
        ...
        varsTables[activeFuncTable][name[0]] = {'type': tipo, 'kind': "local",
                                                "dirV": localMemory.getAddressPointers(tipo)}
        # clear the temp vars
        name.clear()
...
def cleanGlobals():
    ...
    global pTypes
    global pAssigns
    global pAssignsTypes
    global pCompOperands
    global pCompOperators
    global pCompTypes
    global quads
    global pilaDim
    global quadCont
    ...
    global globalMemory
    global localMemory
    global tempMemoryLocal
    global tempMemoryGlobal
    global constantsMemory
    global pointersMemoryGlobal
    global pointersMemoryLocal
    global globalTempsTable
    global localTempsTable
    global constTable
    global arrId
    global arrType
...
def runProgram(inputData):
    ...
    f.write("")
    f.close()
    print("INPUT DATA", len(inputData))
    try:
        if yacc.parse(inputData) == "COMPILA":
            print("Valid input")
            vm = VirtualMachine.VirtualMachine(quads, funcsDir, varsTables, constTable, globalTempsTable,
                                               localTempsTable)
            vm.debug_structs()
            vm.translate_quads()
            vm.debug_structs()
