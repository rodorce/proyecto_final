    ...
    def executeProgram(self):
        ...
        while cont < len(quads):
            ...
            if quads[cont][0] == 2:  # +
                ...
                cont = self.pContext[len(self.pContext) - 1][
                           'quadToReturn'] - 1  # Regresar a cuadruplo guardado en pContext(pero a uno previo por el cont++ de abajo)
                self.pContext.pop()  # delete actual context
                for item in self.pContext[
                    len(self.pContext) - 1]:  # guarda en memoria toods los registros del contexto previo
                    if item != 'quadToReturn':
                        self.saveDataInMemory(item, self.pContext[len(self.pContext) - 1][
                            item])  # save in that address the saved value
                ...
                if q3 == 0:
                    ...
                    self.paramsToSend[self.getFromMemory(q3)] = self.getFromMemory(
                        q1)  # key->num de param, value->valor para ese param
